                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select id="type" name="type"
                                ...
                                <option value="percent" {{ old('type', $promotion->type) == 'percent' ? 'selected' : '' }}>
                                    Giảm giá theo %
                                </option>
                    ...
                    <div class="col-md-6" id="min-order-amount-field" style="display: none;">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                Áp dụng khuyến mãi cho đơn hàng từ mức giá này
                            </small>
                            @error('min_amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" id="max_quantity" name="max_quantity"
                                ...
                                value="{{ old('max_quantity', $promotion -> max_quantity) }}">
                            @error('max_quantity')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" id="start_date" name="start_date"
                                ...
                                value="{{ old('start_date', $promotion->start_date ? date('Y-m-d\TH:i', strtotime($promotion->start_date)) : '') }}" required>
                            @error('start_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" id="end_date" name="end_date"
                                ...
                                value="{{ old('end_date', $promotion->end_date ? date('Y-m-d\TH:i', strtotime($promotion->end_date)) : '') }}" required>
                            @error('end_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div id="product-group">
                        <div class="form-group">
                            ...
                            <div id="selected-products" style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 10px;">
                                <!-- Danh sách sản phẩm đã chọn sẽ được hiển thị ở đây -->
                            </div>
                        </div>
                        <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        // Initialize selectedProducts with existing promotion products
        @foreach($promotion->products as $product)
            if (!selectedProducts[{{ $product->category_id }}]) {
                selectedProducts[{{ $product->category_id }}] = {};
            }
            selectedProducts[{{ $product->category_id }}][{{ $product->product_id }}] = "{{ $product->product_name }}";
        @endforeach
